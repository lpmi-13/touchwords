...
var heightMultiplier = portrait ? 0.7 : 0.75;
var game = new Phaser.Game(width * 0.8, height * heightMultiplier, Phaser.CANVAS, 'gameDiv', {
  preload: preload,
  create: create,
  update: update
});
...
function create() {
  ...
  var background = game.add.tileSprite(
    0,
    0,
    levelVars.backgroundWidth,
    levelVars.backgroundHeight,
    levelVars.background
  );
  ...
  function addScore(sprite) {
    ...
    if (oldText) {
      ...
      var increaseScoreText = game.add.text(
        sprite.x,
        sprite.y,
        '+10',
        { font: '3em Georgia', fill: '#0095DD' },
        scorePool
      );
    }
  ...
  function createWord() {
    ...
    var word = game.add.text(
      game.world.randomX,
      game.world.height,
      words[randomNumber],
      { font: '3em Arial Black', fill: '#c51b7d' },
      wordPool
    );
    ...
    word.body.velocity.setTo(
      game.rnd.integerInRange(levelVars.velocityXlower, levelVars.velocityXhigher),
      game.rnd.integerInRange(levelVars.velocityYlower, levelVars.velocityYhigher)
    );
  ...
  function levelUpTransition() {
    ...
    if (portrait) {
      levelUpText = game.add.text(game.world.centerX, -150, 'You passed the level!!!', {
        font: '2.5em Georgia',
        fill: '#0095DD'
      });
    } else {
    ...
    var tweenTransition = game.add
      .tween(levelUpText)
      .to({ y: game.world.centerY }, 4000, Phaser.Easing.Bounce.Out, true);
...
var bootState = {
  preload: function() {
    game.load.image('preloader', BASE_FILE_PATH + 'assets/images/loading.png');
...
var loadState = {
  preload: function() {
    ...
    game.load.image('logo', BASE_FILE_PATH + 'assets/images/logo.jpg');
    game.load.text('leveldata', BASE_FILE_PATH + 'js/levels.json');
    ...
    game.load.image('diamond', BASE_FILE_PATH + 'assets/images/diamond.png');
...
var splashScreenState = {
  ...
  create: function() {
    ...
    var touchToStart = game.add.text(game.world.centerX, game.world.height - 35, 'touch the screen to start', {
      font: '1.6em Georgia',
      fill: '#0095DD'
    });
    ...
    var textTween = game.add.tween(touchToStart).to({ alpha: 0.25 }, 300, 'Linear', true, 1, -1);
...
var instructionState = {
  ...
  create: function() {
    ...
    var instructionsText = game.add.text(15, 35, instructions, {
      font: '1.75em Georgia',
      fill: '#0095DD',
      wordWrap: true,
    ...
    if (portrait) {
      var continueText = game.add.text(game.world.centerX, game.world.height - 25, 'touch the screen to continue...', {
        font: '1.5em Georgia',
        fill: '#0095DD'
      });
      ...
      var continueText = game.add.text(game.world.centerX, game.world.height - 50, 'touch the screen to continue...', {
        font: '1.5em Georgia',
        fill: '#0095DD'
      });
      continueText.anchor.set(0.5);
...
var bonusState = {
  create: function() {
    ...
    function timeExpired() {
      ...
      var lossText = game.add.text(this.game.world.centerX, this.game.world.centerY, "TIME'S UP", {
        font: timesUpFont,
        fill: '#ff0000'
      });
    ...
    if (portrait) {
      var buttonScaleX = (elementWidth * 0.8) / 53;
      var buttonScaleY = (elementHeight * 0.65) / 40;
    } else {
      var buttonScaleX = (elementWidth * 0.85) / 53;
      var buttonScaleY = (elementHeight * 0.85) / 40;
    }
    ...
    function renderBonusItem() {
      ...
      if (portrait) {
        promptText = game.add.text(game.world.centerX, game.world.height * 0.2, 'correct this word: ', {
          font: '3em Georgia',
          fill: '#dc9a41'
        });
        wordToCorrect = game.add.text(game.world.centerX, game.world.height * 0.3, displayItem, {
          font: '3.25em Georgia',
          fill: '#dc9a41'
        });
      } else {
      ...
      for (var j = 0; j < shuffledWord.length; j++) {
        ...
        var yPos = game.world.height * 0.9 - row * elementHeight;
      ...
      deleteButton = game.add.sprite(game.world.width - screenGutterWidth, game.world.height * 0.9, 'deleteButton');
      ...
      var rectangleWidth = portrait ? game.world.width * 0.85 : game.world.width * 0.65;
      var rectangleHeight = game.world.height * 0.15;
      ...
      if (portrait) {
        displaySpelling = game.add.text(game.world.centerX, game.world.centerY, spellText, {
          font: '4em Georgia',
          fill: '#000000',
          align: 'center'
        });
      ...
      function addBonusScore() {
        ...
        if (oldBonusText) {
          ...
          var increaseBonusScoreText = game.add.text(
            displaySpelling.x,
            displaySpelling.y,
            '+50',
            { font: '3.5em Georgia', fill: '#000000' },
            bonusScorePool
          );
        }
...
var progressState = {
  create: function() {
    ...
    var continueText = game.add.text(
      game.world.centerX,
      game.world.centerY,
      'Touch the screen to continue to the next level!!!',
      { font: '2.5em Georgia', fill: '#0095DD', wordWrap: true, wordWrapWidth: width * 0.65 }
    );
    continueText.anchor.set(0.5);
    game.input.onTap.addOnce(this.start, this);
  },
...
var winState = {
  create: function() {
    ...
    game.stage.backgroundColor = 0x000000;
    var winText = game.add.text(game.world.centerX, game.world.centerY, "THAT'S ALL, YOU WIN!!!", {
      font: '2.5em Georgia',
      fill: '#0095DD',
      wordWrap: true,
    ...
    var finalScoreText = game.add.text(
      game.world.centerX,
      game.world.centerY + game.world.height * 0.25,
      'Final Score: ' + score,
      { font: '2.5em Georgia', fill: '#0095DD' }
    );
    finalScoreText.anchor.set(0.5);

    var continueText = game.add.text(game.world.centerX, game.world.height - 50, 'Touch the screen to play again...', {
      font: '1.5em Georgia',
      fill: '#0095DD',
      wordWrap: true,
      wordWrapWidth: width * 0.65
...
var loseState = {
  create: function() {
    ...
    game.stage.backgroundColor = 0x000000;
    var instructionsText = game.add.text(game.world.centerX, game.world.centerY, 'GAME OVER', {
      font: '2.5em Georgia',
      fill: '#0095DD'
    });
